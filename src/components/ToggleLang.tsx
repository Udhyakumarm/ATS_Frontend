          ...
          >
            <Listbox.Options className="absolute min-w-[150px] top-[100%] right-0 mt-1 max-h-60 w-full overflow-auto rounded-md bg-primary py-1 shadow-normal ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {languages.map((item, itemIdx) => (
                <Listbox.Option
                className={({ active }) =>
                    `relative cursor-pointer select-none py-2 pl-10 pr-4 block group ${
                    active ? 'bg-indigo-100 text-indigo-900' : 'text-white'
                    }`
                }
                value={item}
                key={itemIdx}
                ...
                >
                  ...
                  {lang === item.name ? (
                      <span className={`absolute inset-y-0 left-0 flex items-center pl-3 text-white group-hover:text-indigo-900`}>
