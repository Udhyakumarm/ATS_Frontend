...
function DaysOfMonth({
	...
	const CalendarDay = ({
		...
		<div
			className="group h-[45px] w-[calc(100%/7)] cursor-pointer border md:h-[55px] lg:h-[100px]"
			key={day.toString()}
		>
			<div
				className={
	...
	async function disconnectGoogle() {
		await axiosInstanceAuth2.post("gcal/disconnect-google/").then((res) => {
			if (res.data.res) {
				...
				toastcomp("Google Calendar Successfully Removed.", "success");
				setdissgcal(false);
	...
	return (
		<>
			<div className="flex flex-wrap">
				<div className="w-full bg-white dark:bg-gray-800 md:max-w-[60%] lg:max-w-[75%]">
				...
				<div className="w-full bg-lightBlue dark:bg-gray-900 md:max-w-[40%] lg:max-w-[25%]">
					<div className="p-4">
			...
			<Transition.Root show={dissgcal} as={Fragment}>
				<Dialog as="div" className="relative z-40" initialFocus={cancelButtonRef} onClose={setdissgcal}>
					<Transition.Child
						as={Fragment}
						enter="ease-out duration-300"
						enterFrom="opacity-0"
						...
						<div className="flex min-h-full items-center justify-center p-4 text-center">
							...
							>
								<Dialog.Panel className="relative w-full transform overflow-hidden rounded-[30px] bg-[#FBF9FF] text-left text-black shadow-xl transition-all dark:bg-gray-800 dark:text-white sm:my-8 sm:max-w-lg">
									<div className="flex items-center justify-between bg-gradient-to-b from-gradLightBlue to-gradDarkBlue px-8 py-3 text-white">
										<h4 className="flex items-center font-semibold leading-none">Disconnect Google Calendar</h4>
										<button
											type="button"
											className="leading-none hover:text-gray-700"
											onClick={() => setdissgcal(false)}
										>
											<i className="fa-solid fa-xmark"></i>
										</button>
									</div>
									<div className="p-8">
										<div className="text-sm">
											If You Wanna Disconnect Your Google Calendar That Would Be Delete Your Upcoming & Past Interview
											Events Related Data.
										</div>
