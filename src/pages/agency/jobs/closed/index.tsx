...
export default function JobsDrafted({ atsVersion, userRole, upcomingSoon }: any) {
	...
	useEffect(() => {
		if (search.length > 0) {
			...
			for (let i = 0; i < closedJobs.length; i++) {
				if (closedJobs[i]["jobTitle"].toLowerCase().includes(localSearch)) {
					arr.push(closedJobs[i]);
				}
			}
			setFilterJobs(arr);
		} else {
			setFilterJobs(closedJobs);
		}
	}, [search]);
	...
	return (
		<>
			<Head>
				<title>{t("Words.ClosedJobs")}</title>
			</Head>
			<main>
				<OrgSideBar />
				<OrgTopBar />
						...
						<div className="mx-auto w-full max-w-[980px] px-4 py-8">
							...
							{filterJobs && filterJobs.length > 0 ? (
								...
								<div className="mx-auto w-full max-w-[300px] py-8 text-center">
									...
									<h5 className="mb-4 text-lg font-semibold">{t("Words.ClosedJobs")}</h5>
									<p className="mb-2 text-sm text-darkGray">
										{srcLang === "ja"
											? "終了した求人はありません。終了した求人を管理するには、新しい求人を投稿してください。"
											: "There are no Closed Jobs , Post a New Job to manage closed Jobs"}
									</p>
									<Link
										href={"/agency/jobs/create"}
