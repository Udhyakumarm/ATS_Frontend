	...
	useEffect(() => {
		if (token && token.length > 0) {
			...
			rws.onmessage = function (e) {
				...
				var data = cardActiveData;
				if (fdata["msg_type"] === 1 && parseInt(fdata["user_pk"]) === data["other_user_id"]) {
					console.log("^^^", "MSG_TYPE", 1);
					if (!online.includes(fdata["user_pk"])) {
						setonline([...online, fdata["user_pk"]]);
					}
					// setonline(true);
					// if (one) {
				...
				if (fdata["msg_type"] === 2 && parseInt(fdata["user_pk"]) === data["other_user_id"]) {
					console.log("^^^", "MSG_TYPE", 2);
					const updatedItems = online.filter((existingItem) => existingItem !== data["other_user_id"]);
					setonline(updatedItems);
					// setonline(false);
				}
			};
									...
									<div className="overflow-hidden rounded-normal border bg-white/50 shadow-normal dark:border-gray-600 dark:bg-gray-800/50">
										<Tab.Group>
											...
											<Tab.Panels>
												<Tab.Panel>
													<Tab.Group>
														<Tab.List className={"px-6"}></Tab.List>
														<Tab.Panels>
															<Tab.Panel>
																	...
																	<div className="h-[calc(100vh-264px)] overflow-y-auto p-4 pt-0">
																		{sidebarData.map((data, i) => (
																			<div key={i}>
																				...
																				<ContextMenuTrigger
																					id={`${
																						cardActive && cardActiveData["id"] === data["id"] ? "contextmenu" : ""
																					}`}
																				>
																					<div
																						className={
																					...
																					>
																						<div className="relative h-[50px] w-[50px]">
																							...
																							{/* )} */}
																							{online.includes(data["other_user_id"]) && (
																						...
																						<div className="w-[calc(100%-50px)] pl-3 pt-1">
																							<div className="mb-2 flex justify-between">
																								<h5 className="overflow-hidden truncate text-ellipsis text-sm font-semibold">
																									{data["username"]}
																								</h5>
																				...
																				{cardActive && cardActiveData["id"] === data["id"] && (
																					<ContextMenu
																						id="contextmenu"
																						className="rounded-lg border-2 bg-white py-4 shadow-lg dark:bg-gray-700"
																					>
																						<MenuItem
																							data={{ text: "Mute" }}
