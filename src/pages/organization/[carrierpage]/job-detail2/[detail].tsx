...
export default function CanCareerJobDetail2(props) {
	...
	async function addCandidateProfile(refid: any) {
		if (noAuth && noAuthCount === 1) {
			...
			console.log("^^^", "vendor rating", rdata);
			// toastcomp("VendorCandidate Rating Fetch", "success");
			let rating = rdata.percentage;
			if (rating && typeof rating === "string" && typeof rating != "number") {
				rating = parseInt(rating);
			}
			console.log("rating", rating);

			if ((rdata.version === "enterprise" && rating > 40) || rdata.version != "enterprise") {
				// }
				// else{
				// 	//other version
				// }
	...
	async function getRating(fd: any, refid: any) {
		setocrLoader(true);

		console.log("$", "OCR", "In progress...");
		await axiosInstanceOCR
			.post(`/vendors/job-fit-1/${refid}/`, fd)
			.then(async (res) => {
				var data = res.data;
				console.log("$", "OCR Result", data);
				if (data.version === "enterprise") {
					if (data.qualified === "Perfect") {
						//ocr function run
						const fd2 = new FormData();
						fd2.append("resume", resume);
						fd2.append("refid", refid);
						oceFun1(fd2);
					} else if (data.qualified === "Under Qualified") {
						setAddCand(false);
						setocrLoader(false);
						toastcomp(data.qualified, "error");
						setsadApply(true);
						return false;
					} else if (data.qualified === "Over Qualified") {
						setAddCand(false);
						setocrLoader(false);
						toastcomp(data.qualified, "error");
						setsadApply(true);
	...
	// function checkDis(id: any) {
	// 	if (id != null && document.getElementById(id).checked != null) {
																	...
																	{expid &&
																		expid.map((data, i) => (
																			<article className="border-b last:border-b-0" key={i} id={data}>
																				...
																				<div className="flex flex-wrap items-center text-sm">
																					...
																					<div className="my-2 w-[50%] pl-4">
																						<input
																							...
																							className="w-full rounded-normal border border-borderColor text-sm dark:border-gray-600 dark:bg-gray-700"
																							id={`edate${data}`}
																							// disabled={checkDis(`present${data}`)}
																						/>
																					</div>
																				</div>
																				<textarea
																				...
																				></textarea>
																				{/* <label
																					htmlFor={`present${data}`}
																					className="w-full rounded-normal  text-sm  dark:bg-gray-700"
																				>
																					<input
																						type="checkbox"
																						id={`present${data}`}
																						...
																						className="mb-[3px] mr-2"
																					/>
																					You are currently employed at this company.
																				</label> */}
																			</article>
																		))}
																</div>
