...
export default function Detail() {
	...
	useEffect(()=>{
        if((token.length > 0 && Object.keys(applicantdetail).length === 0 && jobid.length > 0 && canid.length > 0) || (refersh != 0)){
			loadApplicantDetail()
		}
		...
		<>
			...
			<main>
                ...
                <div id="dashboard" className="p-4 lg:p-8">
					<div className="-mx-4">
						<Button label="Back" loader={false} btnType="button" 
							handleClick={()=>{
								setjobid("")
								setcanid("")
								setapplicantdetail({})
							...
							{applicantdetail['Resume'] && applicantdetail['Resume'].map((data, i) => (
								<iframe src={`http://127.0.0.1:8000${data['file']}`} key={i} />
							))}
							{applicantlist && applicantlist.map((data, i) => (
							data['job']['refid'] == jobid && data['user']['erefid'] == canid && 
							<ul key={i} className="list-disc m-4 p-4 w-full rounded-normal bg-white p-6 shadow-normal hover:bg-lightBlue dark:bg-gray-700 dark:hover:bg-gray-600">
								...
								<li>Current Status : <span className="text-lg font-bold">{data['status']}</span></li>
								<li>
								<Button label="Sourced" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Sourced",data["arefid"])
								}}
								/>
								<Button label="Applied" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Applied",data["arefid"])
								}} 
								/>
								<Button label="Phone Screen" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Phone Screen",data["arefid"])
								}} 
								/>
								<Button label="Assement" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Assement",data["arefid"])
								}} 
								/>
								<Button label="Interview" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Interview",data["arefid"])
								}} 
								/>
								<Button label="Offered" loader={false} btnType="button" 
								handleClick={()=>{
									chnageStatus("Offered",data["arefid"])
								}} 
								/>
								<Button label="Hired" loader={false} btnType="button" 
								handleClick={()=>{
